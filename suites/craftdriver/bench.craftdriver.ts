/**
 * craftdriver competitive benchmark suite. Scenarios mirror suites/selenium
 * and suites/wdio exactly (same fixtures, same iteration counts, same
 * pinned chromedriver) so the results are comparable — see ../../README.md
 * for the fairness rules this suite must not violate.
 */
...
describe('craftdriver competitive benchmark', () => {
  afterAll(() => {
  ...
  it('8. heavy network app: navigate + wait-visible', async () => {
    ...
    try {
      ...
      const samples = await sample(
        async () => {
          const [elapsed] = await timed(async () => {
            await browser.navigateTo(`${BASE_URL}/heavy-network.html?run=craftdriver-${run++}`);
  ...
  it('9. screenshot to disk (viewport)', async () => {
    ...
    try {
      ...
      await settle();
      const screenshotPath = path.join(tmpdir(), 'craftdriver-perf-screenshot-craftdriver.png');
      const samples = await sample(
        async () => {
